...
private Stack getWhitePawnSquares(int x, int y, String piece){
  ...
  Move vMove1, vMove2, vMove3, vMove4;
  for(int i=1;i <= 2;i++){
    int tmpx = x;
    int tmpy = y + i;
    ...
    if(!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)){
      ...
      vMove1 = new Move(startingSquare, tmp);
      if(y==1) {
  ...
  for(int i=1;i < 2;i++){
    ...
    if(!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)){
      Square tmp = new Square(tmpx, tmpy, piece);
      vMove4 = new Move(startingSquare, tmp);
      if (!piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) {
        moves.push(vMove4);
      }
  ...
  for(int i=1;i < 2;i++){
    int tmpx1 = x+i;
    int tmpy1 = y+i;
    if(!(tmpx1 > 7 || tmpx1 < 0 || tmpy1 > 7 || tmpy1 < 0)){
      Square tmp = new Square(tmpx1, tmpy1, piece);
      vMove2 = new Move(startingSquare, tmp);
      if(piecePresent(((tmp.getXC()*75)+20), (((tmp.getYC()*75)+20)))){
  ...
  for(int i=1;i < 2;i++){
    int tmpx2 = x-i;
    int tmpy2 = y+i;
    if(!(tmpx2 > 7 || tmpx2 < 0 || tmpy2 > 7 || tmpy2 < 0)){
      Square tmp = new Square(tmpx2, tmpy2, piece);
      vMove3 = new Move(startingSquare,tmp);
      if(piecePresent(((tmp.getXC()*75)+20), (((tmp.getYC()*75)+20)))){
        if(checkWhiteOponent(((tmp.getXC()*75)+20), ((tmp.getYC()*75)+20))){
          moves.push(vMove3);
          break;
...
private void printStack(Stack input){
  ...
  private void humanMoves() {
    ...
    */
    ...
    Stack black = findBlackPieces();// adding stack of all black pieces
    Stack completeMoves = new Stack();
    Move tmp;
    while (!black.empty()) {
...
*/
  ...
  System.exit(0);
    ...
    */
    ...
    else if (AI==1){ //calling the next best move method
      System.out.println("=============================================================");
      Border redBorder = BorderFactory.createLineBorder(Color.RED, 3);
      Move selectedMove = agent.nextBestMove(testing, black);// pasing testing and black through the method
      Square startingPoint = (Square) selectedMove.getStart();
      Square landingPoint = (Square) selectedMove.getLanding();
      int startX1 = (startingPoint.getXC() * 75) + 20;
      int startY1 = (startingPoint.getYC() * 75) + 20;
...
//        }
	...
	*/
    public static void main(String[] args) {
        ...
        AI = n;// used to call each ai agent to the corrosponding n.
